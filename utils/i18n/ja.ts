...
export const ja: Translations = {
  ...
  dividendTax: {
    ...
    pendingActualNoteTemplate: 'MoneyDJ 自動取込：1株 {perShare}、保有 {qty} 株',
  },
  funds: {
    title: '資金管理',
    operations: '操作',
...
* **利息**: 預金または証券口座の利息を記録。
* **借入利息**: 借入利息の支出。**証券口座**では現金減、**負債口座**では借入残高増。

### 借入・負債口座
* **口座種別**: 「口座管理」で「負債（借入）」を選択。年利(%)、借入限度額、連動証券口座を設定可能。
* **借入引出し（推奨）**: 資金管理の**送金**で**負債口座→証券口座**のみ。負債口座への**入金**は併用しない（残高・純投資の重複を避ける）。
* **返済**: 証券口座→負債口座の送金（実際の返済額を記録。元本のみ、または元利合計）。
* **完済の判断**: 負債口座の「借入残高」で判断（資金記録の累計）。**残高ゼロ＝完済**。当初借入額との一致は不要（利息込みの返済総額が当初額を超えるのは正常）。
* **完済後**: 口座・記録は削除不要。「口座管理」「資金管理」で残高・履歴を確認可能。
* **ダッシュボード**: 借入残高**> 0** のときのみ借入カード、返済／スプレッド警告、レバレッジ注記を表示。**残高 0 では借入セクション非表示**。総資産カードの負債・純資産表示も残高があるときのみ。

### 取引種別
* **Buy/Sell**: 一般的な売買。
* **配当再投資 (DRIP)**: 株式の無償割当や配当の再投資などで保有株数が増える。
* **現金配当**: 現金が入り、残高が増える。
...
## 4. よくある質問(FAQ)
Q: 年率リターンはどう計算されますか？
A: ダッシュボードの「年率リターン（XIRR）」は、入出金と現在の総資産に基づく資金加重の年率（不定期キャッシュフロー）です。株価のみの CAGR とは異なります。資産配分シミュレーターの自動取得は CAGR です。
...
Q: 借入口座の設定と引出し・返済の記録は？
A: 「口座管理」で種別「負債（借入）」、年利・限度額（任意）を設定。**引出し**：資金管理→送金→負債口座→証券口座。**返済**：証券口座→負債口座。負債残高は元本（負債側に記録した借入利息を含む）。

Q: 負債口座に「入金」で銀行からの融資を記録しない理由は？
A: 入金と負債→証券の送金を両方記録すると借入残高が二重になることがあります。純投資・XIRRは負債口座の入金を除外しますが、帳簿残高は過大になり得ます。**送金1件のみ**が推奨です。

Q: 借入の完済はどう判断？返済総額が当初借入額を超える場合は？
A: **負債口座の残高がゼロか**で判断します（「返済総額＝当初借入」ではありません）。引出し・負債側の借入利息は残高増、返済は残高減。利息込みで総返済が当初額を超えるのは正常です。銀行の実際の引落に合わせて記録してください。残高 0 でダッシュボードの借入UIは非表示—「口座管理」で残高を確認できます。

Q: 「借入済／限度／使用率」の計算は？
A: 借入残高**> 0** のときのみカード表示。**借入済**＝負債口座の現在残高。**限度**＝設定した借入限度額（口座と同じ通貨）。**使用率**＝借入済÷限度×100%（最大100%表示）。

Q: 「翌月予定利息」の計算は？
A: **翌月予定利息**＝残高×年利(%)÷100÷12。例：200万・年利2.2%→約**3,667**/月。単利・1か月固定残高の**目安**です。

Q: 借入と純投資・XIRR・純資産の関係は？
A: **借入の引出し／返済**（負債↔証券の送金）は**純投資・XIRR**に含まれます。**借入利息**は含みません。**純資産**＝総資産−負債合計。**残高ゼロ後**は借入カード・負債表示・借入関連警告は非表示ですが、過去の引出し／返済は純投資明細に残ります。レバレッジ利用時はXIRRの解釈にご注意ください。

Q: 株式の転送（証券会社Aから証券会社Bへ）はどのように記録しますか？
A: **推奨（一度で完了）**：「記録追加」→ 種別「転出」→ 口座に移出元（A）→ 日付・市場・銘柄・数量を入力 → **転入先口座**（B）を選択して保存。アプリが**転出と転入の2件を同時に**作成します（2件目を手動で足す必要はありません）。自動作成される転入の**手数料は 0**；手数料がある場合は**転出**側に入力してください。単価は平均コストが既定です（市場価格ではなく）。

**補足**：転送後、保有はAから減りBに増えます。現金残高は口座ごとに手数料分が反映されます。
...
- 証券会社または金融機関が提供するリアルタイム見積もりを基準とすることをお勧めします。`,
  ...
  appMessages: {
    ...
    cashFlowCleared: '✅ すべての資金フローを削除しました！',
    historicalSaved: '履歴資産データを更新しました。レポートは実際の価格で補正されました。',
    loginPasswordPlaceholder: 'パスワードを入力',
    confirmClearTxTitle: 'すべての取引を削除しますか？',
    confirmClearTxMessage: 'この操作は元に戻せません。バックアップを確認してください。',
