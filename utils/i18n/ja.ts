...
export const ja: Translations = {
  ...
  dashboard: {
    ...
    accountReturnRateTooltip:
      '累積リターン = 損益 ÷ 分母。純入金優先、≤0なら累計コスト。移管日時価ありで送り側実現・受け側は時価原価。保有なし=終了。',
    accountClosedBadge: '終了',
    unrealizedPL: '含み損益',
    realizedPL: '実現損益',
...
* **利息**: 預金または証券口座の利息を記録。
* **借入利息**: 借入利息の支出。**証券口座**に記録すると現金が減ります（ドロップダウンは証券口座のみ）。
...
Q: 借入口座の作成方法は？
A: **口座管理** → 口座追加 → 種別「負債（借入）」→ 名称・通貨・年利・借入限度額（任意）。初期残高は **0**。
...
Q: 借入利息はどこに記録？
A: **証券口座**。資金管理 → 借入利息 → 証券口座を選択（ドロップダウンは証券口座のみ）。銀行が「元本＋利息」を一括引落する場合は**2件に分ける**：① 送金（証券→負債）で**元本**；② 借入利息で**証券口座**に利息。

Q: 完済の判断は？
A: 負債口座の**借入残高がゼロか**（過払いのマイナスも完済扱い）。利息込みの返済総額が当初借入額を超えるのは正常です。

Q: 完済後に口座を削除する？
A: 不要。口座と履歴は残せます。残高 0 ならダッシュボードの借入セクションは非表示。
...
- 証券会社または金融機関が提供するリアルタイム見積もりを基準とすることをお勧めします。`,
  ...
  transactionForm: {
    ...
    transferBookPrice: '帳簿原価（単価）',
    transferBookPriceHint: '帳簿移管用。下の移管日時価で績效を計算。',
    transferValuationPrice: '移管日の時価（単価）',
    transferValuationHint: 'A口座は決算、B口座はこの価格を取得原価に。現価を初期表示。',
    targetAccount: '入金先口座',
