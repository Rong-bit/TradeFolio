...
* **返済**: 証券口座→負債口座の送金。
* **ダッシュボード**: 借入カード（残高、限度使用率、翌月予定利息）、総資産に負債合計・純資産を表示。
...
Q: 「借入済／限度／使用率」の計算は？
A: **借入済**＝負債口座の現在残高。**限度**＝設定した借入限度額（口座と同じ通貨）。**使用率**＝借入済÷限度×100%（最大100%表示）。

Q: 「翌月予定利息」の計算は？
A: **翌月予定利息**＝残高×年利(%)÷100÷12。例：200万・年利2.2%→約**3,667**/月。単利・1か月固定残高の**目安**です。

Q: 借入と純投資・XIRR・純資産の関係は？
A: **借入の引出し／返済**（負債↔証券の送金）は**純投資・XIRR**に含まれます。**借入利息**は含みません。**純資産**＝総資産−負債合計。
