...
export const zhTW: Translations = {
  ...
  dividendTax: {
    ...
    pendingActualSourceMoneyDj: 'MoneyDJ 實績',
    pendingActualSourceYahoo: 'Yahoo 補入',
    pendingActualPayDate: '發放日',
    pendingActualPerShare: '每股股息',
    pendingActualEstAmount: '估算金額',
    pendingActualAccount: '入帳帳戶',
    pendingActualNoteTemplate: 'MoneyDJ 自動匯入：每股 {perShare}，持股 {qty}',
  },
  funds: {
    title: '資金管理',
    operations: '操作選項',
...
* **利息**: 記錄存款或證券戶利息。
* **信貸利息**: 貸款利息支出；記在**證券戶**時扣現金，記在**負債戶**時增加欠款餘額。

### 信貸／負債專戶
* **帳戶類型**：於「證券戶管理」新增帳戶時，類型選「負債專戶（信貸）」，可設定信貸類型、**年利率 (%)**、**信貸額度**、連動證券戶。
* **信貸撥款（建議做法）**：請用資金管理「**轉帳**」，由**負債戶 → 證券活存**，勿對負債戶再額外記一筆「匯入」，以免帳面欠款與淨投入重複計算。
* **還本**：證券戶 → 負債戶轉帳（金額依銀行實際扣款登打，可為本金、或本息合計）。
* **結清判斷**：以負債戶「欠款餘額」為準（資金紀錄累計）；**餘額歸零即視為還清**，與「當初借款金額」無關（還款總額含利息時通常大於本金）。
* **結清後**：負債專戶與資金紀錄仍保留，無需刪除帳戶；可在「證券戶管理」查餘額、「資金管理」查歷史。
* **儀表板**：負債戶**欠款餘額 > 0** 時才顯示「信貸／負債」卡片（欠款、額度使用率、預計下月利息）、繳款／利差警示，以及淨成本卡上的借貸提示；**餘額為 0 時主畫面不顯示信貸區塊**（如同未借款）。有負債時，總資產卡標題改為**淨資產**、主數字為淨資產，副標僅顯示負債合計；無負債時仍顯示總資產。
...
Q: 如何建立信貸帳戶並登打撥款、還本？
A: **證券戶管理**新增帳戶，類型選「負債專戶（信貸）」，填名稱、幣別、年利率、信貸額度（選填）。**撥款**：資金管理 → 轉帳 → 來源選負債戶、目標選證券戶、金額為撥入金額。**還本**：轉帳由證券戶轉回負債戶。負債戶餘額代表目前欠款本金（含已記入的負債戶信貸利息）。

Q: 為什麼不建議對負債戶用「匯入」記銀行撥款？
A: 若同時記負債戶「匯入」又記「負債戶→證券戶」轉帳，帳面欠款可能重複累加；淨投入／XIRR 則已排除負債戶匯入，但帳本餘額仍可能偏高。正確做法：**只記一筆**負債戶→證券戶轉帳作為信貸撥入。

Q: 如何判斷信貸已還完？還款總額大於當初借款怎麼辦？
A: 程式以**負債戶欠款餘額是否歸零**判斷，不是比對「已還總額＝當初借款」。撥款與記在負債戶的信貸利息會增加餘額；還本（證券戶→負債戶）會減少餘額。實務上還款常為本金+利息，總額大於當初借款是正常的。請依銀行扣款逐筆登打；餘額為 0 時儀表板不顯示信貸區塊，可至「證券戶管理」確認該負債戶餘額。

Q: 儀表板「已借／額度／使用率」如何計算？
A: 僅在該負債戶**欠款餘額 > 0** 時顯示此卡片。**已借**＝該負債戶目前欠款餘額（資金紀錄累計）。**額度**＝帳戶設定的信貸額度（與帳戶同幣別）。**使用率**＝已借 ÷ 額度 × 100%（上限顯示 100%）。未設定額度則不顯示進度條。
...
A: **信貸撥入／還本**（負債戶↔證券戶轉帳）會計入**淨投入**與**XIRR**（視為槓桿資金）；**信貸利息**不計入淨投入。儀表板**總資產**為證券資產＋證券戶現金；**負債合計**為各負債戶欠款換算加總；**淨資產**＝總資產 − 負債合計。**欠款餘額歸零後**，儀表板不顯示信貸卡片、負債合計與借貸相關警示，但歷史撥入／還本仍保留在淨投入明細中供查帳。曾有借貸時解讀 XIRR 請留意槓桿影響。
