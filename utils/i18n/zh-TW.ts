...
### 交易紀錄
記錄買賣、股利與持股轉移。點右上角「記一筆」新增；可透過「批量修改市場」一次調整多筆交易的市場設定。
...
**持股轉移（甲證券 → 乙證券）**
**建議（一次完成）**：「記一筆」→ 類別選 **匯出持股 (Transfer Out)** → 交易帳戶選來源（甲證券）→ 填日期、市場、代號、股數 → 於畫面選擇 **匯入持股目標帳戶**（乙證券）→ 點「確認儲存」。系統會**同時建立** **匯出持股** 與 **匯入持股** 兩筆紀錄；自動建立的匯入持股筆**手續費為 0**，有手續費時請在 **匯出持股** 這筆填寫。價格預設為該帳戶該標的平均成本，請以成本為準、勿用市價。
...
Q: 訪客與會員有何差異？
A: **訪客**可登入並使用「儀表板」、「交易紀錄」、「資金管理」、「證券戶」、「股票拆分」、「配置模擬」等基本功能；儀表板顯示績效摘要卡與持倉明細。**會員**另可解鎖「再平衡」選單，以及累積損益圖、資產配置圖、市場績效、股息熱力圖與 AI 歷史資產校正等進階功能。可於「系統管理」申請訂閱。
