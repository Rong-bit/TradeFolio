...
export const zhTW: Translations = {
  ...
  dashboard: {
    ...
    accountReturnRateTooltip:
      '累積報酬率 = 損益 ÷ 分母 × 100%。分母優先淨入金；若淨入金≤0 則用累計投入成本。轉倉若填「轉移日市值」，匯出戶認列已實現、匯入戶成本以市值為準。無持股標示已結清。',
  ...
  dividendTax: {
    ...
    pendingActualSubtitle: '近 90 天內已除息、尚未在交易紀錄出現的配息；按下「新增」即建立一筆現金股息交易。',
...
Q: 如何建立信貸帳戶？
A: **證券戶管理** → 新增帳戶 → 類型選「負債專戶（信貸）」→ 填名稱、幣別、年利率、信貸額度（選填）。初始餘額請留 **0**。

Q: 銀行撥款到證券戶，怎麼記？
A: 資金管理 → **轉帳** → **負債戶 → 證券戶** → 填撥入金額。**只記這一筆**；勿對負債戶再記「匯入」，以免欠款重複計算。

Q: 如何還本？
A: 資金管理 → **轉帳** → **證券戶 → 信貸戶** → 金額填**本金**部分。信貸戶餘額只追本金，還本金後應歸零。

Q: 信貸利息要記在哪？
A: **證券戶**。資金管理 → 信貸利息 → 選證券戶，會扣現金（下拉選單僅顯示證券戶，不含信貸專戶）。若銀行一次扣「本金＋利息」，請**分兩筆**：① 轉帳（證券戶→信貸戶）記**本金**；② 信貸利息記**證券戶**記利息。

Q: 怎樣算還清了？
A: 以信貸戶**欠款餘額是否歸零**為準（負數代表多付利息，亦視為已結清）。還款總額含利息時，通常大於當初借款本金，這是正常的。

Q: 還清後要刪帳戶嗎？
A: 不用。帳戶與歷史紀錄可保留；餘額為 0 時，儀表板不再顯示信貸區塊。

Q: 儀表板有欠款時怎麼顯示？
A: **信貸專戶「餘額／欠款」欄 > 0**（即欠款本金）：顯示「信貸／負債」卡片，主卡改為**淨資產**（總資產 − 負債合計）。該欄 **= 0**：不顯示信貸區塊，主卡恢復**總資產**。
...
A: **建議（一次完成）**：「記一筆」→ 類別選「匯出持股」→ 交易帳戶選來源（甲證券）→ 填日期、市場、代號、股數 → 選**匯入目標帳戶**（乙證券）→ 確認。系統會建立匯出／匯入兩筆；匯入手續費為 0，手續費填在匯出。**帳面成本**預設均價；**轉移日市值**預設現價（可改），用於各帳戶累積報酬率（A 結案、B 接手成本）。
...
- 建議以證券公司或金融機構提供的即時報價為準。`,
  ...
  transactionForm: {
    ...
    transferBookPrice: '帳面成本單價',
    transferBookPriceHint: '用於移轉帳面與手續費；績效結案請填下方轉移日市值。',
    transferValuationPrice: '轉移日市值單價',
    transferValuationHint: '績效用：A 戶依此結案報酬，B 戶以此為接手成本。預設帶入現價，可改。',
