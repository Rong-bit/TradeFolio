...
* **還本**：證券戶 → 負債戶轉帳。
* **儀表板**：負債戶**欠款餘額 > 0** 時才顯示「信貸／負債」卡片（欠款、額度使用率進度條、預計下月利息）與相關警示；餘額為 0 時儀表板不顯示信貸區塊。總資產卡於有負債時亦顯示負債合計與淨資產。
...
A: **已借**＝該負債戶目前欠款餘額（資金紀錄累計）。**額度**＝帳戶設定的信貸額度（與帳戶同幣別）。**使用率**＝已借 ÷ 額度 × 100%（上限顯示 100%）。未設定額度則不顯示進度條。
...
A: **信貸撥入／還本**（負債戶↔證券戶轉帳）會計入**淨投入**與**XIRR**（視為槓桿資金）；**信貸利息**不計入淨投入。儀表板**總資產**為證券資產＋證券戶現金；**負債合計**為各負債戶欠款換算加總；**淨資產**＝總資產 − 負債合計。年化報酬含借貸時請留意解讀。
