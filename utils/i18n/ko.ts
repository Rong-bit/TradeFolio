...
* **대출 이자**: 대출 이자 지출. **증권 계좌**는 현금 감소, **부채 계좌**는 차입 잔액 증가.

### 신용·부채 계좌
* **계좌 유형**: 「계좌 관리」에서 「부채(차입)」 선택. 연이율(%), 신용 한도, 연동 증권 계좌 설정.
* **차입 인출(권장)**: 자금 관리 **송금**으로 **부채 계좌→증권 계좌**만. 부채 계좌 **입금**은 함께 쓰지 마세요.
* **상환**: 증권 계좌→부채 계좌 송금(실제 상환액 기록. 원금만 또는 원리금 합계).
* **완납 판단**: 부채 계좌 「차입 잔액」 기준(자금 기록 누계). **잔액 0 = 완납**. 당초 차입액과 총 상환액 일치 여부와 무관(이자 포함 시 총 상환이 더 큰 것은 정상).
* **완납 후**: 계좌·기록 삭제 불필요. 「계좌 관리」「자금 관리」에서 잔액·이력 확인.
* **대시보드**: 차입 잔액 **> 0** 일 때만 차입 카드, 상환/스프레드 경고, 레버리지 안내 표시. **잔액 0이면 차입 섹션 숨김**. 총자산 카드의 부채·순자산도 잔액 있을 때만.
...
Q: 차입 계좌 설정과 인출·상환 기록은?
A: 「계좌 관리」에서 「부채(차입)」, 연이율·한도(선택) 설정. **인출**: 자금 관리→송금→부채→증권. **상환**: 증권→부채. 부채 잔액은 원금(부채 측에 기록한 대출 이자 포함).

Q: 부채 계좌에 「입금」으로 대출 받는 금액을 기록하지 않는 이유는?
A: 입금과 부채→증권 송금을 모두 기록하면 잔액이 이중 계산될 수 있습니다. 순투입·XIRR은 부채 입금을 제외하지만 장부 잔액은 과대할 수 있습니다. **송금 한 건**만 권장합니다.

Q: 차입 완납은 어떻게 판단? 총 상환이 당초 차입액보다 크면?
A: **부채 계좌 잔액이 0인지**로 판단합니다(「총 상환＝당초 차입」이 아님). 인출·부채 측 대출 이자는 잔액 증가, 상환은 감소. 이자 포함 총 상환이 더 큰 것은 정상입니다. 은행 실제 출금에 맞춰 기록하세요. 잔액 0이면 대시보드 차입 UI 숨김—「계좌 관리」에서 잔액 확인.

Q: 「차입／한도／사용률」 계산은?
A: 차입 잔액 **> 0** 일 때만 카드 표시. **차입**＝부채 계좌 현재 잔액. **한도**＝설정한 신용 한도. **사용률**＝차입÷한도×100%(최대 100%).

Q: 「다음 달 예상 이자」 계산은?
A: **예상 이자**＝잔액×연이율(%)÷100÷12. 예: 200만·2.2%→약 **3,667**/월. 단리·한 달 고정 잔액 **참고치**입니다.

Q: 차입과 순투입·XIRR·순자산 관계는?
A: **차입 인출／상환**(부채↔증권 송금)은 **순투입·XIRR**에 포함. **대출 이자**는 제외. **순자산**＝총자산−부채 합계. **잔액 0 후**에는 차입 카드·부채 표시·차입 경고 숨김, 과거 인출/상환은 순투입 내역에 유지. 레버리지 사용 시 XIRR 해석에 주의.
