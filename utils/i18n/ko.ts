...
export const ko: Translations = {
  ...
  dashboard: {
    ...
    accountReturnRateTooltip:
      '누적 수익률 = 손익 ÷ 분모. 순입금 우선; ≤0이면 누적 원가. 이관 시 시가 반영. 보유 없음=종료.',
    accountClosedBadge: '종료',
    unrealizedPL: '미실현 손익',
    realizedPL: '실현 손익',
...
* **대출 이자**: 대출 이자 지출. **증권 계좌**에 기록하면 현금이 감소합니다(드롭다운은 증권 계좌만).
...
Q: 차입 계좌는 어떻게 만드나요?
A: **계좌 관리** → 계좌 추가 → 유형 「부채(차입)」→ 이름·통화·연이율·신용 한도(선택). 초기 잔액은 **0**.
...
Q: 대출 이자는 어디에?
A: **증권 계좌**. 자금 관리 → 대출 이자 → 증권 계좌 선택(드롭다운은 증권 계좌만). 은행이 「원금+이자」를 한 번에 출금하면 **두 건으로**: ① 송금(증권→부채) **원금**; ② 대출 이자 **증권 계좌**에 이자.
...
Q: 「다음 달 예상 이자」 계산은?
A: **예상 이자**＝현재 차입 잔액 × 연이율(%) ÷ 100 ÷ 12. 예: 200만·2.2%→약 **3,667**/월. 단리·한 달 고정 잔액 **참고치**; 은행은 일할·월중 상환으로 다를 수 있음. @ 는 계좌 연이율.
...
- 증권사 또는 금융 기관이 제공하는 실시간 시세를 기준으로 하는 것을 권장합니다.`,
  ...
  transactionForm: {
    ...
    transferBookPrice: '장부 원가(단가)',
    transferBookPriceHint: '장부 이관용. 아래 이관일 시가로 수익률 계산.',
    transferValuationPrice: '이관일 시가(단가)',
    transferValuationHint: '출고 계좌 결산, 입고 계좌 취득 원가.',
