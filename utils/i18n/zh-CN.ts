...
zhCN.stockSplit = { title: '股票拆分管理', hint: '交易记录维持当时登录的股数与价格；拆分事件仅在计算持仓与损益时依生效日自动调整。若生效日后的交易已改为拆分后股数，请勿重复登录。', ticker: '股票代码', tickerPlaceholder: '例如：2330、AAPL', market: '市场', effectiveDate: '生效日期', sharesFrom: '拆分前股数', sharesTo: '拆分后股数', splitRatio: '拆分比例', note: '备注', notePlaceholder: '例如：2024 年 1 月股票拆分', addButton: '新增股票拆分事件', listTitle: '已设定的股票拆分', totalCount: '共 {count} 笔', emptyList: '尚无拆分事件', tickerRequired: '请输入股票代码', dateRequired: '请选择生效日期', invalidRatio: '拆分前后股数须为正整数', duplicateError: '同一股票、市场与生效日已存在拆分事件', addSuccess: '已新增股票拆分事件', deleteSuccess: '已删除股票拆分事件', deleteConfirmTitle: '确认删除拆分事件', deleteConfirmMessage: '确定要删除 {ticker}（{date}）的拆分事件吗？' };
