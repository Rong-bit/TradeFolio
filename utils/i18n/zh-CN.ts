...
* **利息**：记录存款或证券户利息。
* **信贷利息**：贷款利息支出；记在**证券户**时扣现金，记在**负债户**时增加欠款余额。

### 信贷／负债专户
* **账户类型**：于「证券户管理」新增账户时，类型选「负债专户（信贷）」，可设定信贷类型、**年利率 (%)**、**信贷额度**、联动证券户。
* **信贷拨款（建议做法）**：请用资金管理「**转账**」，由**负债户 → 证券活存**，勿对负债户再额外记一笔「汇入」，以免账面欠款与净投入重复计算。
* **还本**：证券户 → 负债户转账（金额依银行实际扣款登打，可为本金、或本息合计）。
* **结清判断**：以负债户「欠款余额」为准（资金记录累计）；**余额归零即视为还清**，与「当初借款金额」无关（还款总额含利息时通常大于本金）。
* **结清后**：负债专户与资金记录仍保留，无需删除账户；可在「证券户管理」查余额、「资金管理」查历史。
* **仪表板**：负债户**欠款余额 > 0** 时才显示「信贷／负债」卡片、缴款／利差警示及净成本卡上的借贷提示；**余额为 0 时主画面不显示信贷区块**。总资产卡仅于有负债时显示负债合计与净资产。
...
Q: 如何建立信贷账户并登打拨款、还本？
A: **证券户管理**新增账户，类型选「负债专户（信贷）」，填名称、币别、年利率、信贷额度（选填）。**拨款**：资金管理 → 转账 → 来源选负债户、目标选证券户、金额为拨入金额。**还本**：转账由证券户转回负债户。负债户余额代表目前欠款本金（含已记入的负债户信贷利息）。

Q: 为什么不建议对负债户用「汇入」记银行拨款？
A: 若同时记负债户「汇入」又记「负债户→证券户」转账，账面欠款可能重复累加；净投入／XIRR 则已排除负债户汇入，但账本余额仍可能偏高。正确做法：**只记一笔**负债户→证券户转账作为信贷拨入。

Q: 如何判断信贷已还清？还款总额大于当初借款怎么办？
A: 程序以**负债户欠款余额是否归零**判断，不是比对「已还总额＝当初借款」。拨款与记在负债户的信贷利息会增加余额；还本（证券户→负债户）会减少余额。实务上还款常为本金+利息，总额大于当初借款是正常的。请依银行扣款逐笔登打；余额为 0 时仪表板不显示信贷区块，可至「证券户管理」确认该负债户余额。

Q: 仪表板「已借／额度／使用率」如何计算？
A: 仅在该负债户**欠款余额 > 0** 时显示此卡片。**已借**＝该负债户目前欠款余额（资金记录累计）。**额度**＝账户设定的信贷额度（与账户同币别）。**使用率**＝已借 ÷ 额度 × 100%（上限显示 100%）。未设定额度则不显示进度条。
