...
zhCN.dividendTax = {
  ...
  pendingActualNoteTemplate: 'MoneyDJ 自动导入：每股 {perShare}，持股 {qty}',
};
zhCN.funds = {
  ...zhTW.funds,
  title: '资金管理',
  ...
  last30Days: '最近30天',
  thisYear: '今年',
  confirmClearAll: '确认清空筛选的 {count} 笔资金记录？',
  confirmClearAllMessage: '此操作将删除目前筛选条件下的入金、出金、转账与利息记录，且无法恢复。建议先备份数据。',
  confirmClear: '确认清空',
};
zhCN.accounts = {
  ...zhTW.accounts,
...
Q: 如何建立信贷账户？
A: **证券户管理** → 新增账户 → 类型选「负债专户（信贷）」→ 填名称、币别、年利率、信贷额度（选填）。初始余额请留 **0**。

Q: 银行拨款到证券户，怎么记？
A: 资金管理 → **转账** → **负债户 → 证券户** → 填拨入金额。**只记这一笔**；勿对负债户再记「汇入」，以免欠款重复计算。

Q: 如何还本？
A: 资金管理 → **转账** → **证券户 → 信贷户** → 金额填**本金**部分。信贷户余额只追本金，还本金后应归零。

Q: 信贷利息要记在哪？
A: **证券户**。资金管理 → 信贷利息 → 选证券户，会扣现金（下拉选单仅显示证券户，不含信贷专户）。若银行一次扣「本金＋利息」，请**分两笔**：① 转账（证券户→信贷户）记**本金**；② 信贷利息记**证券户**记利息。

Q: 怎样算还清了？
A: 以信贷户**欠款余额是否归零**为准（负数代表多付利息，亦视为已结清）。还款总额含利息时，通常大于当初借款本金，这是正常的。

Q: 还清后要删账户吗？
A: 不用。账户与历史记录可保留；余额为 0 时，仪表板不再显示信贷区块。

Q: 仪表板有欠款时怎么显示？
A: **信贷专户「余额／欠款」栏 > 0**（即欠款本金）：显示「信贷／负债」卡片，主卡改为**净资产**（总资产 − 负债合计）。该栏 **= 0**：不显示信贷区块，主卡恢复**总资产**。
...
## 4. 常见问题 (FAQ)
Q: 如何计算年化报酬率？
A: 仪表板「真实年化（XIRR）」为资金加权年化，依汇入／汇出记录与目前总资产计算（不定期现金流）；与「仅看价涨跌」的 CAGR 不同。资产配置模拟器内自动查询的年化为 CAGR。
...
Q: 仪表板「已借／额度／使用率」如何计算？
A: 仅在该信贷专户**「余额／欠款」栏 > 0** 时显示此卡片。**已借**＝该栏数字（资金记录累计之欠款本金）。**额度**＝账户设定的信贷额度（与账户同币别）。**使用率**＝已借 ÷ 额度 × 100%（上限显示 100%）。未设定额度则不显示进度条。
...
A: **信贷拨入／还本**（负债户↔证券户转账）会计入**净投入**与**XIRR**（视为杠杆资金）；**信贷利息**不计入净投入。仪表板**总资产**为证券资产＋证券户现金；**负债合计**为各负债户欠款换算加总；**净资产**＝总资产 − 负债合计。**欠款余额归零后**，仪表板不显示信贷卡片、负债合计与借贷相关警示，但历史拨入／还本仍保留在净投入明细中供查账。曾有借贷时解读 XIRR 请留意杠杆影响。
...
zhCN.appMessages = {
  ...
  guestLoginNotice: '已为您登录「非会员模式」。\n\n部分进阶功能可能受限。若遇到问题，请至「系统管理」使用问题回报通知开发者。',
  contactSubject: 'TradeView 问题回报',
  contactBody: 'Hi 管理员,\n\n我的账号是: {user}\n\n【问题描述】\n请在此说明您遇到的 Bug 或问题：\n\n\n\n谢谢协助。',
  contactSentSuccess: '问题回报已送出！开发者将尽快查阅，谢谢您的反馈。',
  contactMailtoFallback: '无法直接送出回报，已为您开启 Gmail 撰写邮件。请补充问题后按「发送」。',
  contactStaticPagesHint: '已在新分页开启 Gmail 撰写邮件，并复制回报内容到剪贴板。\n\n请在 Gmail 分页补充问题说明后按「发送」。\n\n若未开启新分页，请手动前往 mail.google.com 或寄信至：{email}',
