...
### 交易记录
记录买卖、股利与持股转移。点右上角「记一笔」新增；可透过「批量修改市场」一次调整多笔交易的市场设定。
...
**持股转移（甲证券 → 乙证券）**
**建议（一次完成）**：「记一笔」→ 类别选 **汇出持股 (Transfer Out)** → 交易账户选来源（甲证券）→ 填日期、市场、代号、股数 → 在界面选择 **汇入持股目标账户**（乙证券）→ 点「确认保存」。系统会**同时建立** **汇出持股** 与 **汇入持股** 两笔记录；自动建立的汇入持股笔**手续费为 0**，有手续费时请在 **汇出持股** 这笔填写。价格默认为该户该标的平均成本，请以成本为准、勿用市价。
...
Q: 信贷利息要记在哪？
A: 「证券户」。「资金管理」→「信贷利息」→ 选证券户，会扣现金（下拉选单仅显示证券户，不含信贷专户）。若银行一次扣「本金＋利息」，请**分两笔**：①「转账」（证券户→信贷户）记**本金**；②「信贷利息」记**证券户**记利息。
...
Q: 如何计算年化报酬率？
A: 「仪表板」的 **真实年化（XIRR）** 为资金加权年化，依汇入／汇出记录与目前总资产计算（不定期现金流）；与 **仅看价涨跌** 的 CAGR 不同。「配置模拟」内自动查询的年化为 CAGR。

Q: 汇率如何设定？
A: 可在右上角设定全域 USD/TWD 汇率，或在「转账」时指定当下汇率。

Q: 访客与会员有何差异？
A: **访客**可登入并使用「仪表板」、「交易记录」、「资金管理」、「证券户」、「股票拆分」、「配置模拟」等基本功能；仪表板显示绩效摘要卡与持仓明细。**会员**另可解锁「再平衡」菜单，以及累积损益图、资产配置图、市场绩效、股息热力图与 AI 历史资产校正等进阶功能。可于「系统管理」申请订阅。
...
A: 当**基准币为台币 (TWD)** 时，「仪表板」的 **详细统计** 区块会显示海外所得试算进度（非台湾市场之已实现损益与股利合计），并参考 100 万申报门槛与 670 万基本扣除额试算最低税负。**仅供报税规划参考**，未含保险给付等其他所得，不构成税务建议。
